...
# uv is installed in stage-1 for peid; keep it build-time.
...
# Make pixi available in login (non-interactive) shells too.
# Ubuntu's default ~/.bashrc returns early for non-interactive shells, so ~/.profile is the
# right place for PATH additions.
ensure_line "${profile_path}" 'export PATH="$HOME/.pixi/bin:$PATH"'
chown "peid:${primary_group}" "${profile_path}" || true

# Bun global installs.
#
# tavily package (required)
run_as_user peid "${bun_bin}" add -g tavily-mcp@latest

# optional packages: may require npm credentials or registry access (scoped packages often return 404 when unauthenticated).
# Keep the build portable by default; opt into strict behavior with PEI_STRICT_BUN_GLOBAL=1.
