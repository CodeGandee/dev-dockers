...
This folder provides a small **in-container bridge** so that **Claude Code** (Anthropic API)
...
Projects can provide their own launcher (example: `dockers/infer-dev` uses
`installation/stage-2/custom/check-and-run-litellm.sh`) while reusing these system scripts.
